	...
	else if (id == 0) // Caso Filho
	{
		for(i = 0; i < 1000; i++)
		{
			*p += 2; // Soma dois à variável compartilhada na memória
			printf("%d - Filho val: %d\n", i, *p); // Exibe valor de *p durante execução do Processo Filho
		}
	}
	else
	{
		for(i = 0; i < 1000; i++)
		{
			*p += 3; // Soma dois à variável compartilhada na memória
			printf("%d - Pai val: %d\n", i, *p); // Exibe valor de *p durante execução do Processo Pai
		}
